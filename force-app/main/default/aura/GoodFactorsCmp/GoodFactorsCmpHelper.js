({
    ...
    navigateTo : function(component){
      ...
      if (agencyIndex!= -1)
      {
        url = "https://" + window.location.hostname + "/apex/GoodFactorsCompletePage?scontrolCaching=1&id=" + component.get('v.recordId');
      } else if (pathCrm) {    
          url = "/crm/s/fattori-del-bene?recordId=" + component.get('v.recordId');
      } 
          else{
              url = "https://" + window.location.hostname + "/apex/GoodFactorsCompletePage?id=" + component.get('v.recordId');
          }
