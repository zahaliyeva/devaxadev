            ...
            if(state === 'SUCCESS'){
                ...
                if(column_metadata_wrapper.error_message){
                    ...
                    for(let i = 0; i < field_names.length; i++){
                         if(field_names[i]!='UrlName' && field_names[i]!='Line_of_Business_Relevance__c'){
                        table_columns.push({
                    ...
                    if(field_names[i] == 'Line_of_Business_Relevance__c'){
                      if(UserProfile == 'AAI - Vendite Avanzato' || UserProfile == 'AAI - Vendite Base' || UserProfile == 'AAI - Vendite No Accordo Digitale'){

                            table_columns.push({
                            is_selection_column: false,
                            field_name: field_names[i],
                            field_api_name: column_metadata_wrapper.column_metadata[field_names[i]].field_api_name,
                            field_label: 'Area/Rilevanza',
                            field_type: column_metadata_wrapper.column_metadata[field_names[i]].field_type,
                            field_override_type: column_metadata_wrapper.column_metadata[field_names[i]].field_override_type,
                            field_is_reference: column_metadata_wrapper.column_metadata[field_names[i]].field_is_reference,
                            field_is_sortable: column_metadata_wrapper.column_metadata[field_names[i]].field_is_sortable
        ...
        action.setCallback(this, function(response){
            ...
            if(state === 'SUCCESS'){
                ...
                if(sobject_wrapper.error_message){
                    ...
                    //component.set("v.UserProfile",'AAI - Vendite Avanzato');
                    //console.log(component.get("v.UserProfile")+UserProfile!!!);
                         for(j = 0; j < sobject_wrapper.sobjects.length; j++){
                             if(isMobility){
                                 sobject_wrapper.sobjects[j].Line_of_Business_Relevance__c = sobject_wrapper.sobjects[j].Line_of_Business_Relevance__c.replace('/resource','/crm/resource');
                             }
                            ...
                            sobject_wrapper.sobjects[j].Line_of_Business_Relevance__c = sobject_wrapper.sobjects[j].Line_of_Business_Relevance__c.replace('/resource','/agenzie/resource'); 
                            if(!isMobile){
//                          LB.    
//                          sobject_wrapper.sobjects[j].Id = 'articles/News/'+sobject_wrapper.sobjects[j].UrlName;
                            sobject_wrapper.sobjects[j].Id = 'articles/Knowledge/'+sobject_wrapper.sobjects[j].UrlName;
                        }
                         }
                    }
                ...
                for(let j = 0; j < table_columns.length; j++){
                    if(table_columns[j].is_selection_column){
                        ...
                        row.push({
                            ...
                            value: value,
                            field_api_name : table_columns[j].field_api_name,
